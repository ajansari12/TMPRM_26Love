-- Migration: Platform Admin, Shared Third Party Registry, and Configurable Workflows
-- Addresses: Super admin capabilities, cross-FI vendor linkage, workflow customization
...
CREATE TABLE platform_audit_log (
  ...
  resource_type TEXT NOT NULL, -- 'tenant', 'platform_admin', 'global_config', etc.
  resource_id UUID,
  organization_id UUID REFERENCES organizations(id), -- If action is org-specific
  details JSONB,
  ip_address INET,
  user_agent TEXT,
  created_at TIMESTAMPTZ DEFAULT NOW()
...
CREATE INDEX idx_platform_audit_log_org ON platform_audit_log(organization_id);
CREATE INDEX idx_platform_audit_log_created ON platform_audit_log(created_at DESC);

-- ============================================================================
-- SHARED THIRD PARTY REGISTRY (Cross-FI Vendor Linkage)
-- ============================================================================

-- Global third party registry (platform-level vendor master)
-- This allows linkage across FIs while maintaining tenant isolation
CREATE TABLE global_third_parties (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  
  -- Identity
  legal_name TEXT NOT NULL,
  trading_names TEXT[], -- Array of known trading names
  lei TEXT UNIQUE, -- Legal Entity Identifier (if available)
  duns_number TEXT, -- D&B number
  ...
  -- Company info
  company_type TEXT, -- public, private, government, non_profit
  industry_codes TEXT[], -- NAICS/SIC codes
  website TEXT,
  
  -- Risk indicators (platform-level aggregated)
  global_risk_score INTEGER, -- Aggregated from tenant assessments
  ...
  verification_source TEXT, -- 'manual', 'd_and_b', 'bloomberg', etc.
...
CREATE INDEX idx_global_third_parties_search ON global_third_parties USING gin(search_vector);
CREATE INDEX idx_global_third_parties_lei ON global_third_parties(lei) WHERE lei IS NOT NULL;
CREATE INDEX idx_global_third_parties_name ON global_third_parties(LOWER(legal_name));
...
-- This is the linkage table showing which FIs use which third parties
CREATE TABLE vendor_global_links (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  organization_id UUID NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
  vendor_id UUID NOT NULL REFERENCES vendors(id) ON DELETE CASCADE, -- Tenant's vendor record
  global_third_party_id UUID NOT NULL REFERENCES global_third_parties(id),
  
  -- Linkage metadata
  link_confidence TEXT DEFAULT 'confirmed', -- confirmed, suggested, manual
...
CREATE INDEX idx_vendor_global_links_org ON vendor_global_links(organization_id);
CREATE INDEX idx_vendor_global_links_global ON vendor_global_links(global_third_party_id);

-- View showing cross-FI concentration (anonymized for non-admins)
CREATE OR REPLACE VIEW third_party_concentration AS
SELECT 
  gtp.id AS global_third_party_id,
  gtp.legal_name,
  gtp.headquarters_country,
  COUNT(DISTINCT vgl.organization_id) AS fi_count,
  COUNT(DISTINCT vgl.vendor_id) AS total_relationships,
  ARRAY_AGG(DISTINCT o.institution_type) AS institution_types
FROM global_third_parties gtp
JOIN vendor_global_links vgl ON gtp.id = vgl.global_third_party_id
JOIN organizations o ON vgl.organization_id = o.id
GROUP BY gtp.id, gtp.legal_name, gtp.headquarters_country;
...
CREATE TABLE workflow_templates (
  ...
  -- Template identity
  name TEXT NOT NULL,
  code TEXT NOT NULL, -- 'vendor_onboarding', 'contract_review', 'incident_response'
  description TEXT,
    ...
    steps: [
      {
        ...
        "defense_line": "1a",
        "actions": ["submit", "save_draft", "cancel"],
        "required_fields": ["vendor_legal_name", "service_category"],
        "next_step": "1b_review"
      },
      {
        "order": 2,
        ...
        "next_step_on_return": "submission"
      },
      ...
    ]
  */
  ...
  /*
    conditions: {
      "skip_1b_for_low_risk": true,
      "require_2nd_line_for_critical": true,
      "contract_value_threshold_for_escalation": 1000000
    }
  */
...
CREATE UNIQUE INDEX idx_workflow_templates_unique_default 
...
CREATE TABLE workflow_step_configs (
  ...
  auto_assign_to JSONB, -- {"type": "role", "value": "risk_coordinator"} or {"type": "user", "value": "uuid"}
  fallback_assignees UUID[], -- If auto-assign fails
  
  -- SLA settings
  sla_hours INTEGER,
  warning_hours INTEGER,
  escalation_hours INTEGER,
  escalation_to JSONB, -- Who to escalate to
  
  -- Required actions/validations
  required_attachments TEXT[], -- ['business_case', 'risk_assessment']
  required_approvals INTEGER DEFAULT 1, -- Number of approvals needed
  
  -- Skip conditions
  skip_conditions JSONB, -- Conditions under which this step can be skipped
  /*
    skip_conditions: {
      "when": [
        {"field": "preliminary_risk_tier", "operator": "in", "value": ["low", "informational"]},
        {"field": "estimated_contract_value_cad", "operator": "<", "value": 50000}
      ],
      "logic": "AND"
    }
  */
...
-- ============================================================================
-- WORKFLOW ACTIONS & APPROVALS TRACKING
-- ============================================================================
...
CREATE TABLE workflow_approvals (
  ...
  workflow_type TEXT NOT NULL, -- 'onboarding', 'contract', 'incident'
  workflow_item_id UUID NOT NULL, -- ID of the specific request/item
  step_id TEXT NOT NULL,
  
  -- Approval details
  approver_id UUID NOT NULL REFERENCES auth.users(id),
  decision TEXT NOT NULL, -- 'approved', 'rejected', 'abstained', 'delegated'
  delegated_to UUID REFERENCES auth.users(id),
  
  -- Context
  conditions TEXT[], -- Any conditions attached to approval
  notes TEXT,
  risk_assessment_score INTEGER, -- Approver's risk assessment if applicable
...
CREATE INDEX idx_workflow_approvals_item ON workflow_approvals(workflow_type, workflow_item_id);
CREATE INDEX idx_workflow_approvals_approver ON workflow_approvals(approver_id);
CREATE INDEX idx_workflow_approvals_pending ON workflow_approvals(organization_id, decided_at) WHERE decided_at IS NULL;
...
CREATE TABLE user_delegations (
  ...
  -- Scope
  delegation_type TEXT NOT NULL DEFAULT 'all', -- 'all', 'specific_workflows', 'approval_only'
  workflow_types TEXT[], -- If specific, which workflows
  
  -- Time bounds
  effective_from TIMESTAMPTZ NOT NULL,
  effective_until TIMESTAMPTZ,
  reason TEXT, -- 'vacation', 'leave', 'temporary_assignment'
...
CREATE INDEX idx_user_delegations_active ON user_delegations(delegator_id) 
WHERE is_active = true;
...
BEGIN
  ...
  SELECT 
    ...
    o.created_at,
    COALESCE((o.settings->>'is_active')::boolean, true) AS is_active
...
-- Global third parties RLS (platform admins only for write, tenants can read)
...
CREATE POLICY "Users can view their org's vendor links"
...
CREATE POLICY "Users can manage their org's vendor links"
...
CREATE POLICY "Users can view their org's templates and platform defaults"
ON workflow_templates FOR SELECT
USING (
  organization_id IS NULL -- Platform defaults viewable by all
...
CREATE POLICY "Users can view their org's step configs"
...
CREATE POLICY "Users can view their org's delegations"
...
USING (
  (delegator_id = auth.uid() AND organization_id IN (SELECT get_user_organization_ids()))
  OR is_platform_admin()
);
...
COMMENT ON TABLE workflow_approvals IS 'Individual approval records for multi-approval workflows';
