/*
  ...
  ## Summary
  Drops and recreates all functions with a mutable search_path, adding
  SET search_path = '' and fully-qualifying all schema references to prevent
  search path injection attacks in security definer functions.

  ## Steps
  1. Drop dependent policy and cascade-drop all affected functions (removes triggers too)
  2. Recreate all 10 functions with SET search_path = ''
  3. Recreate all 4 triggers that were dropped by cascade
  4. Recreate the dependent RLS policy

  ## Affected Functions
  1. is_onboarding_request_approved(uuid)
  2. user_can_create_vendors(uuid)
  3. vendor_has_assessment(uuid)
  4. validate_vendor_status() [trigger]
  5. link_vendor_to_onboarding() [trigger]
  6. update_onboarding_template_updated_at() [trigger]
  7. validate_vendor_creation() [trigger]
  8. create_vendor_from_onboarding(uuid)
  9. clone_onboarding_request(uuid, uuid)
  10. backfill_legacy_vendors()
*/

-- ============================================================
-- Step 1: Drop dependent RLS policy first, then functions (CASCADE drops triggers)
-- ============================================================
...
-- ============================================================
-- Step 2: Recreate all functions with SET search_path = ''
-- ============================================================
...
BEGIN
  ...
  INSERT INTO public.onboarding_requests (
    organization_id, legal_name, trading_name, country, city, province_state, website,
    description, service_category, provider_type, business_unit, is_critical,
    contract_value_cad, primary_contact_name, primary_contact_email, primary_contact_phone,
    ...
    source_request.country, source_request.city, source_request.province_state,
    source_request.website, source_request.description, source_request.service_category,
    source_request.provider_type, source_request.business_unit, source_request.is_critical,
    source_request.contract_value_cad, source_request.primary_contact_name,
    source_request.primary_contact_email, source_request.primary_contact_phone,
...
BEGIN
  ...
  LOOP
    INSERT INTO public.onboarding_requests (
      organization_id, legal_name, trading_name, country, city, province_state,
      website, description, service_category, provider_type, business_unit,
      is_critical, contract_value_cad, primary_contact_name, primary_contact_email,
      primary_contact_phone, handles_sensitive_data, has_system_access, uses_subcontractors,
      business_justification, requested_by, status, is_retrospective, is_draft,
      submitted_at, approved_at
    ) VALUES (
      vendor_record.organization_id, vendor_record.legal_name, vendor_record.legal_name,
      ...
      vendor_record.business_unit, COALESCE(vendor_record.is_critical, false),
      vendor_record.contract_value_cad, vendor_record.primary_contact_name,
      vendor_record.primary_contact_email, vendor_record.primary_contact_phone,
...
-- ============================================================
-- Step 3: Recreate triggers that were dropped by CASCADE
-- ============================================================
